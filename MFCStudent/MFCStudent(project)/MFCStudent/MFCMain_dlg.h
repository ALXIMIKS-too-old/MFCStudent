...
public:
	...
	afx_msg void OnNMClickList(NMHDR *pNMHDR, LRESULT *pResult);   //  event handler: Click on LIST

	CListCtrl m_list_ctrl;			 // CListCtrl of LIST
	CList<stud> m_students;			 // List of students
	CImageList m_image_list;		 // icons for LIST	
	CString  DataStorageFile;        // name of serialization file
	CString m_storage_name;			 // info for storage

	void FillList();				 //  rebuild  of LIST
	void OnOK();					 //  event handler: Enter 
	void OnCancel();				 //  event handler: exit 
	void Serialize(const CString &DataStorageFile, int flag, int RandItemsNumber = 20); //  Serialization of  CList<stud> m_students;	 	

	static int CALLBACK CompareFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);	 // compare using ItemDATA	
	int read_init(const CString &config_name, const CString &default_config_name, CString &DataStorageFile, 
